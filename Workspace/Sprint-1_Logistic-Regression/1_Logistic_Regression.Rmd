...
set.seed(178)
sample <- caret::createDataPartition(diabetic_data2$classes, 
                                     p = 0.7, 
                                     list = FALSE)
...
diab_ret.glm.eval <- eval_mod(diab_ret.glm, test)

#double check colinearity
# >>> Get names of selected parameters (remove 'Intercept' and factors) <<< #
selected_variables <- names(coef(diab_ret.glm$finalModel))[c(-1,-2)]

# >>> Create correlation matrix <<<# 
correlation_matrix <- diabetic_data2 %>%
...
set.seed(178)
diab_ret.glm.lasso <- train(classes ~ .,
                            data = train,
                            method = "glmnet", 
                            direction = "both",
                            trControl = train_method,
                            ...
                            metric = "ROC",
                            trace = F)
lasso <- eval_mod(diab_ret.glm.lasso, test)

set.seed(178)
diab_ret.glm.ridge <- train(classes ~ .,
                            data = train,
                            method = "glmnet", 
                            direction = "both",
                            trControl = train_method,
                            ...
                            metric = "ROC",
                            trace = F)
ridge <- eval_mod(diab_ret.glm.ridge, test)

set.seed(178)
diab_ret.glm.enet  <- train(classes ~ .,
                            data = train,
                            method = "glmnet", 
                            direction = "both",
                            trControl = train_method,
                            ...
                            metric = "ROC",
                            trace = F)
enet <- eval_mod(diab_ret.glm.enet, test)
```